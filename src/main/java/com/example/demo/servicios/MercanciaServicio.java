...
public class MercanciaServicio {
	...
	public Mercancia eliminarMercancia(Integer id, Integer idUsuario) {
		Mercancia mercancia = this.getMercanciaById(id);
		if (mercancia != null) {
			Usuario usuario = this.usuarioRepositorio.findById(idUsuario).get();
		
				if (mercancia.getUsuarioCreado().getId() ==idUsuario) {
					this.mercanciaRepositorio.delete(mercancia);
					return mercancia;
				} else {
					throw new ApiRequestExcepcion("solo lo puede eliminar el usuario que lo creo");
				}
	...
	public List<Mercancia> listar(Integer id) {
		...
		for (Mercancia mercancia : mercancias) {
			if (mercancia.getUsuarioCreado().getId() == id) {
